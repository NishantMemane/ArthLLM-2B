"""
NSE Insider Trading PDFs — Corporate Announcements endpoint  (PRODUCTION)
Endpoint: /api/corporate-announcements?index=equities&from_date=&to_date=
Filters announcements for insider trading / SAST / PIT related filings.
Captures PDF URLs from attchmntFile field.

Real desc categories (verified via probe):
  - "Disclosure under SEBI Takeover Regulations" (SAST)
  - "Trading Window" (PIT)
  - "Structural Digital Database" (PIT compliance)
  + any row where attchmntText contains insider/SAST/PIT keywords

• 30-day windows (high volume: ~12K announcements/month)
...
CSV_PATH  = os.path.join(BASE, "nse_insider_trading_pdfs.csv")
PROG_PATH = os.path.join(LOGS, "nse_progress_insider_pdfs.json")
...
# Categories that are directly insider/SAST related
INSIDER_DESC = {
...
INSIDER_KW = re.compile(
    r"insider\s*trad|sast|prohibit.*insider|"
    r"regulation\s*7|reg\.?\s*7\b|reg\.?\s*29|regulation\s*29|"
    r"takeover|pit\b|substantial\s*acq",
    re.IGNORECASE,
)
...
def is_insider(a):
    """Check if this announcement is insider/SAST/PIT related."""
    desc = (a.get("desc") or "").strip()
    if desc in INSIDER_DESC:
        return True
    text = (a.get("attchmntText") or "")
    combined = f"{desc} {text}"
    return bool(INSIDER_KW.search(combined))
...
def main():
    ...
    pbar = tqdm(total=len(pending), desc="nse_insider_pdfs", unit="win",
    ...
    def process(idx, wf, wt):
        ...
        pbar.update(1)
        pbar.set_postfix_str(f"{total_new:,} insider / {total_scan:,} scanned")
