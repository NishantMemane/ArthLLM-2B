"""
NSE Bonus & Split — UNIFIED Scraper  (PRODUCTION)
Two-pass approach:
  Pass 1: /api/corporates-corporateActions → structured data (ex_date, ratio, etc.)
  Pass 2: /api/corporate-announcements   → PDF URLs + announcement text
Merges both into a single CSV: nse_bonus_split.csv
...
CSV_PATH  = os.path.join(BASE, "nse_bonus_split.csv")
PROG_PATH = os.path.join(LOGS, "nse_progress_bonus_split.json")
...
# ── Unified CSV columns ───────────────────────────────────────────────────
CSV_COLUMNS = [
    "source",                                           # "ACTION" or "ANNOUNCEMENT"
    "symbol", "company", "series", "isin", "industry",
    "subject", "action_type", "ratio",
...
# ── Filters ────────────────────────────────────────────────────────────────
BONUS_SPLIT_DESC = {"Bonus", "Stock Split", "Sub-Division"}

BONUS_SPLIT_KW = re.compile(
    r"bonus\s*issue|bonus\s*share|stock\s*split|sub[\s-]*division|"
    r"split.*face\s*value|split.*fv|face\s*value.*split|"
    r"sub[\s-]*divid|bonus.*ratio|bonus.*entitlement",
    re.IGNORECASE,
)

RATIO_BONUS = re.compile(r"(\d+)\s*:\s*(\d+)")
RATIO_SPLIT = re.compile(
    r"(?:Rs\.?\s*|₹\s*)(\d+).*?(?:Rs\.?\s*|₹\s*|to\s*(?:Rs\.?\s*|₹\s*))(\d+)",
    re.IGNORECASE,
)
...
def classify_action(subject):
    ...
    if "SPLIT" in p or "SUB-DIVISION" in p or "SUBDIVISION" in p: return "SPLIT"
    return ""

def is_bonus_split_action(subject):
    return classify_action(subject) != ""

def is_bonus_split_ann(a):
    desc = (a.get("desc") or "").strip()
    if desc in BONUS_SPLIT_DESC: return True
    text = a.get("attchmntText") or ""
    return bool(BONUS_SPLIT_KW.search(f"{desc} {text}"))

def parse_ratio(subject, action_type):
    s = (subject or "").strip()
    if action_type == "BONUS":
        m = RATIO_BONUS.search(s)
        return f"{m.group(1)}:{m.group(2)}" if m else ""
    if action_type == "SPLIT":
        m = RATIO_SPLIT.search(s)
        return f"{m.group(1)}→{m.group(2)}" if m else ""
    return ""
...
class NSESession:
    def __init__(self, sid=0, referer=REFERER_ACT):
        ...
        self.session.headers.update(HEADERS); self.last_warm = 0
        self._warm_up()
...
# ── Row converters ─────────────────────────────────────────────────────────
def action_to_row(a):
    """Corporate Actions record → unified row."""
    subject = (a.get("subject") or "").strip()
    atype = classify_action(subject)
    return [
        "ACTION",
        clean(a.get("symbol")), clean(a.get("comp")),
        clean(a.get("series")), clean(a.get("isin")), "",
        subject, atype, parse_ratio(subject, atype),
...
def ann_to_row(a):
    """Corporate Announcements record → unified row."""
    desc = clean(a.get("desc"))
    text = clean(a.get("attchmntText"))
    atype = classify_action(f"{desc} {text}")
    ratio = parse_ratio(f"{desc} {text}", atype)
    return [
        "ANNOUNCEMENT",
        clean(a.get("symbol")), clean(a.get("sm_name")),
        "", clean(a.get("sm_isin")), clean(a.get("smIndustry")),
        "", atype, ratio,
        "", "", "", "",   # ex/rec/bc dates (not in announcements)
        "", "",           # face_value, broadcast_date
...
def main():
    global first_write_flag
    print("=" * 65)
    print("NSE BONUS & SPLIT — UNIFIED SCRAPER")
    ...
    # Pass 1: Corporate Actions → structured data
    n1 = run_pass(
        pass_name="ACTIONS", api_url=ACTIONS_URL, referer=REFERER_ACT,
        window_days=90,
        filter_fn=lambda a: is_bonus_split_action((a.get("subject") or "")),
        row_fn=action_to_row,
        dedup_fn=lambda a: f"A|{(a.get('symbol') or '').strip()}|{(a.get('exDate') or '').strip()}",
        done_w=done_w,
    )

    # Pass 2: Announcements → PDF URLs
    n2 = run_pass(
        pass_name="ANNOUNCE", api_url=ANNOUNCE_URL, referer=REFERER_ANN,
        window_days=30,
        filter_fn=is_bonus_split_ann,
        row_fn=ann_to_row,
